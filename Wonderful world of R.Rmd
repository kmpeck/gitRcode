...
Excitingly, you can also embed code from other programming languages in your R Markdown document. For example:
...
We can change the bins manually, by increasing or decreasing the number of breaks.

```{r}
par(mfrow=c(2,1))
bins <- seq(min(x), max(x), length.out=10+1)
hist(x, breaks=bins, col="darkgray", border="white", 
     main="old faithful waiting time (10 bins)", xlab="Minutes")
bins <- seq(min(x), max(x), length.out=30+1)
hist(x, breaks=bins, col="darkgray", border="white", 
     main="old faithful waiting time (30 bins)", xlab="Minutes")
```

Shiny allows the user to change the number of bins in real-time. This uses the input from the user (number of bins), and the *renderPlot()* function to allow the plot to update automatically.
